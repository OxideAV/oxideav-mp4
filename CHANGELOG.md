...
- Round 343 — typed §10 sample-group description entries (`sample_group_entries` module): `roll` / `prol` RollRecoveryEntry (signed `roll_distance`) — the first of the base-spec grouping-type decoders/builders layered over the opaque `sgpd` blob surface, mirroring the `cenc::parse_seig` precedent. `SampleGroupGroupingType` routes a `grouping_type` FourCC to its §10 variant (ISO/IEC 14496-12 §10.1.1)
